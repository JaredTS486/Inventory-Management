...
uses
  ...
  StdCtrls, Decl, Lbltypes, Lblfunctions;
...
begin
  ...
  if RowName = 'Justify' then begin
    if ObjectInfo.ObjType = ltBARCODE
  ...
  if RowName = 'Link' then begin
    ...
    // represented in the label
     ...
     for Ind := 1 to ObjectCount do
       if POBJECTINFO(ObjectList + SizeOf(TOBJECTINFO)*(Ind - 1))^.ObjType
         in [ltTEXT, ltADDRESS, ltCOUNTER]
         then lb.Items.Add( StrPas (POBJECTINFO(ObjectList + SizeOf(TOBJECTINFO)*(Ind - 1))^.ObjName));
  end;
